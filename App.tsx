...
import { DesignItem, CostTier, QuantityDiscount, CalculationResult, PackedDesign } from './types';
...
const MASTER_KEY = 'graficapro_v4_stable';
...
const App: React.FC = () => {
  ...
  const getDiscountForQty = useCallback((qty: number) => {
    const discount = appData.quantityDiscounts.find((d: QuantityDiscount) => qty >= d.minQty && qty <= d.maxQty);
  ...
  const updateField = (field: keyof AppDataType, value: any) => {
    setAppData((prev: AppDataType) => ({ ...prev, [field]: value }));
  };
  ...
  const removeDesign = (id: string) => {
    updateField('designs', appData.designs.filter((d: DesignItem) => d.id !== id));
  };
  ...
  const getColorForDesign = (originalId: string) => {
    const index = appData.designs.findIndex((d: DesignItem) => d.id === originalId);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 font-sans text-slate-700">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-slate-400 tracking-widest uppercase">Producción Estable</span>
                <div className="flex items-center gap-1 text-emerald-500 text-[9px] font-black uppercase">
                  <CheckCircle2Icon size={10} /> {lastSaved ? `Auto-Guardado ${lastSaved}` : 'Conectado'}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2"><Spacing className="text-indigo-400" size={14} /> Espaciado (cm)</label>
          ...
          <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200">
            ...
            <div className="space-y-5">
              <input type="text" placeholder="Nombre..." value={newDesign.name} onChange={e => setNewDesign({...newDesign, name: e.target.value})} className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-4 focus:border-indigo-500 outline-none transition font-bold text-slate-900" />
          ...
          <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200">
             <div className="flex items-center justify-between mb-6">
               <h2 className="text-slate-900 font-bold text-sm flex items-center gap-2 uppercase tracking-widest"><Tag className="text-indigo-400" /> Tarifas</h2>
             ...
             <div className="space-y-3">
               {appData.costTiers.map((tier: CostTier, idx: number) => (
                 <div key={tier.id} className="flex gap-2 items-center bg-slate-50 p-3 rounded-2xl border border-slate-100 group">
                   <input type="number" className="w-16 bg-white border border-slate-200 rounded-xl p-2.5 text-[11px] font-bold text-center" value={tier.minLargo} onChange={e => {
                     const val = Number(e.target.value);
                     const nt = appData.costTiers.map((t: CostTier, i: number) => i === idx ? { ...t, minLargo: val } : t);
                     updateField('costTiers', nt);
                   }} />
                   <span className="text-slate-300 font-black text-xs px-1">~</span>
                   <input type="number" className="w-16 bg-white border border-slate-200 rounded-xl p-2.5 text-[11px] font-bold text-center" value={tier.maxLargo} onChange={e => {
                     const val = Number(e.target.value);
                     const nt = appData.costTiers.map((t: CostTier, i: number) => i === idx ? { ...t, maxLargo: val } : t);
                     updateField('costTiers', nt);
                   }} />
                   <div className="flex-1 text-right font-black text-indigo-600 text-sm">$<input type="number" className="w-20 bg-transparent text-right outline-none" value={tier.precioPorCm} onChange={e => {
                     const val = Number(e.target.value);
                     const nt = appData.costTiers.map((t: CostTier, i: number) => i === idx ? { ...t, precioPorCm: val } : t);
                     updateField('costTiers', nt);
                   }} /></div>
                   <button onClick={() => updateField('costTiers', appData.costTiers.filter((t: CostTier) => t.id !== tier.id))} className="text-slate-300 hover:text-rose-500 p-2 opacity-0 group-hover:opacity-100 transition-all"><Trash size={16}/></button>
                 </div>
               ))}
             </div>
          </section>
          ...
          <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200">
             <div className="flex items-center justify-between mb-6">
               <h2 className="text-slate-900 font-bold text-sm flex items-center gap-2 uppercase tracking-widest"><Layers className="text-emerald-400" /> Descuentos</h2>
             ...
             <div className="space-y-3">
               {appData.quantityDiscounts.map((discount: QuantityDiscount, idx: number) => (
                 <div key={discount.id} className="flex gap-2 items-center bg-slate-50 p-3 rounded-2xl border border-slate-100 group">
                   <input type="number" className="w-16 bg-white border border-slate-200 rounded-xl p-2.5 text-[11px] font-bold text-center" value={discount.minQty} onChange={e => {
                     const val = Number(e.target.value);
                     const nd = appData.quantityDiscounts.map((d: QuantityDiscount, i: number) => i === idx ? { ...d, minQty: val } : d);
                     updateField('quantityDiscounts', nd);
                   }} />
                   <span className="text-slate-300 font-black text-xs px-1">~</span>
                   <input type="number" className="w-16 bg-white border border-slate-200 rounded-xl p-2.5 text-[11px] font-bold text-center" value={discount.maxQty} onChange={e => {
                     const val = Number(e.target.value);
                     const nd = appData.quantityDiscounts.map((d: QuantityDiscount, i: number) => i === idx ? { ...d, maxQty: val } : d);
                     updateField('quantityDiscounts', nd);
                   }} />
                   <div className="flex-1 text-right font-black text-emerald-600 text-sm"><input type="number" className="w-10 bg-transparent text-right outline-none" value={discount.discountPercent} onChange={e => {
                     const val = Number(e.target.value);
                     const nd = appData.quantityDiscounts.map((d: QuantityDiscount, i: number) => i === idx ? { ...d, discountPercent: val } : d);
                     updateField('quantityDiscounts', nd);
                   }} />%</div>
                   <button onClick={() => updateField('quantityDiscounts', appData.quantityDiscounts.filter((d: QuantityDiscount) => d.id !== discount.id))} className="text-slate-300 hover:text-rose-500 p-2 opacity-0 group-hover:opacity-100 transition-all"><Trash size={16}/></button>
                 </div>
               ))}
             </div>
          </section>
        ...
        <div className="lg:col-span-8 space-y-8">
          <section className="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-200 relative overflow-hidden">
            ...
            <div className="relative bg-slate-950 rounded-[2.5rem] min-h-[450px] overflow-auto flex justify-center p-16 custom-scrollbar shadow-2xl border-[10px] border-slate-900">
              {packingResult.totalLength > 0 ? (
                ...
                >
                  {packingResult.packed.map((p: PackedDesign) => {
                ...
                <div className="flex flex-col items-center justify-center text-slate-700 opacity-20 py-24">
                  ...
                  <p className="mt-6 font-black uppercase tracking-[0.4em] text-xs">Sin diseños</p>
                </div>
              )}
            </div>
          </section>
          ...
          <section className="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-200">
            <div className="flex items-center gap-4 mb-10">
              ...
              <h2 className="font-black text-xl text-slate-900 tracking-tight">Análisis de Costos</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-separate border-spacing-y-4">
                <thead>
                  <tr className="text-slate-400">
                    <th className="pb-4 px-6 text-[10px] font-black uppercase tracking-widest">Ítem</th>
                    <th className="pb-4 px-4 text-[10px] font-black uppercase tracking-widest text-right">Prod. Un</th>
                    <th className="pb-4 px-4 text-[10px] font-black uppercase tracking-widest text-right">Precio Un</th>
                    <th className="pb-4 px-4 text-[10px] font-black uppercase tracking-widest text-right">Costo Tot</th>
                    <th className="pb-4 px-6 text-[10px] font-black uppercase tracking-widest text-right text-indigo-500">Venta Tot</th>
                  </tr>
                </thead>
                <tbody>
                  {appData.designs.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-24 text-center text-slate-300 font-black uppercase tracking-[0.3em] italic text-xs">Esperando carga...</td>
                    </tr>
                  ) : (
                    appData.designs.map((design: DesignItem) => {
                      const res = calculateDetails(design);
                      const color = getColorForDesign(design.id);
                      return (
                        <tr key={design.id} className="bg-slate-50/50 hover:bg-slate-100 transition-all duration-300 rounded-2xl group">
            ...
            {appData.designs.length > 0 && (
              <div className="mt-12 p-10 bg-slate-900 rounded-[3rem] text-white flex flex-col md:flex-row justify-between items-center gap-10 shadow-2xl border border-white/5 relative overflow-hidden">
                ...
                <div className="text-center md:text-right relative z-10">
                  ...
                  <p className="text-6xl font-black text-emerald-400 tracking-tighter drop-shadow-lg">
                    ${appData.designs.reduce((acc: number, d: DesignItem) => acc + calculateDetails(d).totalClientPrice, 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </p>
                </div>
              </div>
            )}
      ...
      <footer className="mt-24 text-center text-slate-300 text-[10px] py-16 font-black uppercase tracking-[0.5em] border-t border-slate-200">
        GraficaPro v4.0 &bull; Persistencia Stable 2024
      </footer>
    </div>
  );
};
